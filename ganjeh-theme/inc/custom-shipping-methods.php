...
/**
 * Add "روش‌های ارسال" tab to product data tabs
 */
function ganjeh_shipping_restrictions_product_tab($tabs) {
    $tabs['ganjeh_shipping'] = [
        'label'    => __('روش‌های ارسال', 'ganjeh'),
        'target'   => 'ganjeh_shipping_product_data',
        'class'    => [],
        'priority' => 71,
    ];
    return $tabs;
}
add_filter('woocommerce_product_data_tabs', 'ganjeh_shipping_restrictions_product_tab');

/**
 * Add tab icon
 */
function ganjeh_shipping_restrictions_tab_icon() {
    echo '<style>#woocommerce-product-data ul.wc-tabs li.ganjeh_shipping_options a::before{content:"\f312";font-family:dashicons;}</style>';
}
...
function ganjeh_save_product_shipping_restrictions($post_id) {
    ...
    } elseif (isset($_POST['product-type'])) {
